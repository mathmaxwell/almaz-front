const ru = {
	backToMainPage: 'Вернуться на главную страницу',
	error404Title: 'Страница не найдена',
	error404Message:
		'Страница, которую вы ищете, не существует или была перемещена.',
	or: 'или',
	system_login: 'Вход в систему',
	create_account: 'Создать аккаунт',
	register: 'Регистрация',
	login: 'Логин',
	purchases: 'Покупки',
	announcements: 'Объявления',
	light_mode: 'Светлый режим',
	dark_theme: 'Темный режим',
	uzbek: 'Узбекский',
	russian: 'Русский',
	logout_of_system: 'Выйти из системы',
	about: 'О нас',
	game_title: 'Название игры',
	donation_name: 'Название доната',
	price: 'Цена',
	purchase_time: 'Время покупки',
	balance_was: 'На счету было',
	balance_became: 'На счету стало',
	status: 'Статус',
	purchase_history: 'История покупок',
	payment_history: 'История оплат',
	payment_amount: 'Сумма оплаты',
	received_time: 'Время получения',
	contact_us: 'Связаться с нами',
	contact_message: 'Мы всегда на связи! Выберите удобный способ общения',
	add_game: 'Добавить игру',
	update_game: 'Обновить игру',
	delete_game: 'Удалить игру',
	save: 'Сохранить',
	image: 'Изображение',
	name: 'Имя',
	upload_image: 'Загрузить изображение',
	current_or_new_image: 'Текущее или новое изображение',
	image_preview: 'Предварительный просмотр изображения',
	title_ru: 'Название на русском',
	title_uz: 'Название на узбекском',
	description_ru: 'Описание на русском',
	description_uz: 'Описание на узбекском',
	delete: 'Удалить',
	update: 'Обновить',
	add: 'Добавить',
	add_to_cart: 'Добавить в корзину',
	buy_now: 'Купить сейчас',
	title: 'Название',
	add_advertisement: 'Добавить объявление',
	home_page: 'Главная страница',
	bot_id: 'ID бота',
	place: 'Место',
	top: 'Верх',
	bottom: 'Низ',
	stop: 'Остановить',
	wallet: 'Кошелек',
	payment_method: 'Порядок оплаты',
	payment_instruction:
		'Вам необходимо знать сумму оплаты. Минимальная сумма — 1000 сум. Если выбранная вами сумма уже занята, введите другую сумму или подождите.',
	payment_booking:
		'После выбора суммы она считается забронированной за вами. Вы должны произвести оплату в течение 5 минут.',
	payment_timeout:
		'Если вы не выполните оплату в течение 5 минут, выбранную вами сумму может забронировать другой клиент.',
	payment_error_rules: `Оплату необходимо производить точно до каждой цифры забронированной суммы.
Например: вы хотите оплатить 5000 сум, но эта сумма занята. Тогда нажмите оплату на сумму 5001 сум и переведите на карту, указанную сайтом, не 5000 сум, а именно 5001 сум.`,
	remember: `Помните: если вы всё выполните правильно, оплата будет подтверждена быстро.
❌ Подтверждение оплаты не производится вручную (не зависит от человека)`,
	card_number: 'Номер карты',
	add_card: 'Добавить карту',
	how_to_pay: 'Как оплатить',
	no_cards_added: 'Карты пока не добавлены',
	add_new_card: 'Добавить новую карту',
	book_now: 'Забронировать',
	finished: 'Завершено',
	pending: 'В ожидании',
	som: 'сум',
	pending_payments: 'Платежи в ожидании',
	payments: 'Платежи',
	accept: 'Принять',
	decline: 'Отказать',
	confirm_accept: 'Вы уверены, что хотите принять?',
	confirm_delete: 'Вы уверены, что хотите удалить?',
	time: 'Время',
	created_by: 'Создано (кем)',
	balance: 'Баланс',
	type_of_card: 'Тип карты',
	count: 'Количество',
	payment_is_busy: 'Платеж занят',
	you_already_have_an_order: 'У вас уже есть заказ',
	watch_video: 'Смотреть видео',
	selected_amount: 'Выбранная сумма',
	day: 'День',
	copy_number: 'Копировать номер',
	full_information: 'Полная информация',
	view_information: 'Смотреть информацию',
	add_to_favorites: 'В избранное',
	main_image: 'Основное изображение',
	auxiliary_image_instruction: 'Вспомогательное изображение (инструкция)',
	video: 'Видео',
	upload_video: 'Загрузить видео',
	actions: 'Действия',
	buy: 'Купить',
	no_saved_purchases: 'У вас пока нет сохраненных покупок',
	users: 'Пользователи',
	user_name: 'Имя пользователя',
	password: 'Пароль',
	user_token: 'Токен пользователя',
	reset_filters: 'Сбросить фильтры',
	balance_from: 'Баланс от ...',
	data_load_error: 'Ошибка загрузки данных',
	nothing_found_filters: 'Ничего не найдено по заданным фильтрам',
	no_users_yet: 'Пользователей пока нет',
	rows_per_page: 'Строк на странице',
	from: 'из',
	more: 'Больше',
	update_balance: 'Обновить баланс',
	delete_user_confirmation:
		'Вы действительно хотите удалить пользователя? После этого все его данные будут удалены',
	enter_amount_to_add_or_remove:
		'Напишите сумму, которую вы хотите добавить (удалить) пользователю',
	change_amount_confirmation: 'Вы точно хотите изменить сумму на',
	invalid_amount: 'Сумма введена неверно',
	empty: 'Пусто',
	instructions: 'Инструкции',
	filled_by: 'Кем заполнен',
	support_notice: 'Если вы что-то не понимаете, сообщите в техподдержку',
	server_id: 'ID сервера',
	player_id: 'ID игрока',
	promo_code: 'Промокод',
	enter_promo_code: 'Введите промокод',
	discount_must_be_greater_than_zero: 'Скидка должна быть больше 0',
	percentage_discount_cannot_exceed_100:
		'Процентная скидка не может быть больше 100%',
	invalid_end_date: 'Некорректная дата окончания',
	promo_code_created_successfully: 'Промокод успешно создан!',
	error_creating_promo_code: 'Ошибка при создании промокода',
	create_promo_code: 'Создание промокода',
	discount: 'Скидка',
	discount_type: 'Тип скидки',
	fixed_amount: 'Фиксированная сумма',
	percentage: 'Процент',
	max_discount_amount_percent_only: 'Максимальная сумма скидки (только для %)',
	not_applicable: 'Не применяется',
	discount_limit_in_money: 'Ограничение скидки в деньгах',
	min_order_amount: 'Минимальная сумма заказа',
	end_date_optional: 'Дата окончания',
	total_usage_limit: 'Общее количество использований',
	usage_limit_per_user: 'Использований на одного пользователя',
	loading: 'Загрузка',
	create_promo_code_action: 'Создать промокод',
	number_of_fields: 'Количество полей',
	one_field: 'Одно поле',
	two_fields: 'Два поля',
	top_up: 'Пополнение',
	must_register:
		'Для использования этой услуги вам необходимо зарегистрироваться',
	user_price: 'Цена для пользователей',
	selected_people_price: 'Цена для выбранных людей',
	super_users: 'Суперпользователи',
	show_password: 'Показать пароль',
	change_as_superuser: 'Вы хотите изменить это как superUser?',
	change_as_regular_user: 'Вы хотите изменить это как простой user?',
	select_game: 'Выберите игру',
	try_on: 'Примерить',
	game_name: 'Название игры',
	price_for_all: 'Цена для всех',
	superuser_price: 'Цена для superUser',
	dollar_rate: 'Курс доллара',
	statistics: 'Статистика',
	start_date: 'Дата начала',
	end_date: 'Дата окончания',
	deposits: 'Пополнения',
	operations: 'Операций',
	clients: 'Клиентов',
	net_profit: 'Чистая прибыль (пополнения − покупки)',
	overview: 'Обзор',
	total: 'Всего',
	total_deposits: 'Всего пополнений',
	buyers: 'Покупателей',
	top_selling_games: 'Топ игры по продажам',
	no_data: 'Нет данных',
	top_selling_donations: 'Топ донаты по продажам',
	by_games: 'По играм',
	by_donations: 'По донатам',
	total_purchases: 'Итого покупки',
	admin_deposits: 'Пополнения от админа',
	operations_count: 'Количество операций',
	average_amount: 'Средняя сумма',
	payment_system_deposits: 'Пополнения через платежные системы',
	units: 'шт',
	no_deposits: 'Нет пополнений',
	error_insufficient_balance: 'Недостаточно средств на балансе',
	error_user_not_found: 'Пользователь не найден',
	error_offer_not_found: 'Предложение не найдено',
	error_game_not_found: 'Игра не найдена',
	error_invalid_price: 'Некорректная цена',
	error_no_server_id: 'Не указан Server ID',
	error_provider_balance: 'Сервис временно недоступен, попробуйте позже',
	error_provider_threshold: 'Сервис временно недоступен, попробуйте позже',
	error_unknown: 'Произошла ошибка, попробуйте позже',
	error_wait: 'Подождите перед повторной попыткой',
	error_bad_request: 'Некорректный запрос. Проверьте данные и попробуйте снова',
	error_database: 'Ошибка базы данных. Попробуйте позже',
	error_getting_game: 'Ошибка при получении данных игры. Попробуйте позже',
	error_invalid_bot_id: 'Некорректный Bot ID. Обратитесь в поддержку',
	error_provider_not_supported: 'Провайдер данной игры не поддерживается',
	error_invalid_player_id: 'Неверный ID игрока. Проверьте правильность ввода',
	error_title: 'Ошибка',
	donation: 'Донат',
	donation_dashboard: 'Панель донатов',
	total_donations: 'Всего донатов',
	donation_amount: 'Сумма донатов',
	unique_donors: 'Уникальных донатеров',
	avg_donation: 'Средний донат',
	order_statuses: 'Статусы заказов',
	Completed: 'Выполнено',
	in_progress: 'В процессе',
	canceled: 'Отменено',
	Refunded: 'Возвращено',
	partial: 'Частично',
	donations_by_game: 'Донаты по играм',
	donations_by_product: 'Донаты по товарам',
	recent_donations: 'Последние донаты',
	order_number: 'Номер заказа',
	donor: 'Донатер',
	check_status: 'Проверить статус',
	status_loading: 'Загрузка статуса...',
	status_error: 'Ошибка',
	all_statuses: 'Все статусы',
	success_rate: 'Процент успеха',
	top_donors: 'Топ донатеры',
	donations_count: 'Количество донатов',
	username: 'Имя пользователя',
	provider_balances: 'Балансы провайдеров',
	show_all: 'Показать все',
	show_less: 'Свернуть',
	transactions_log: 'Журнал транзакций',
	search_placeholder: 'Поиск по userId, playerId, игре...',
}
export default ru
